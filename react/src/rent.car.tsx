    ...
    try {
      ...
      if (code === 'auth/popup-blocked' || code === 'auth/popup-closed-by-user' || code === 'auth/cancelled-popup-request') {
        try {
          ...
          const redirectCode = (redirectError as { code?: string })?.code || 'unknown';
          console.error('Social redirect login error:', redirectError);
          alert(`Google/Apple login bajarilmadi (${redirectCode}). Firebase > Authentication > Google yoqilganini va Authorized domains to'g'ri ekanini tekshiring.`);
          return;
        }
      }
      console.error('Social popup login error:', error);
